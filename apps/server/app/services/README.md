...
- `analyze_uploaded_etf_bundle()`: analyzes the required uploaded bundle of `ETF1.csv`, `ETF2.csv`, and `prices.csv`
...
- `_build_uploaded_etf_analytics_item()`: computes one uploaded ETF result from one uploaded ETF weights file plus the uploaded prices frame
...
- Uploaded bundle analysis is modeled as one combined operation because both uploaded ETF files must be validated against the same uploaded `prices.csv`.
