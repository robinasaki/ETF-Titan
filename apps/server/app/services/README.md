...
- `list_supported_etfs()`: returns the uploaded ETF catalog
...
- `_build_named_holdings_frame()`: loads one uploaded ETF and the bundled prices dataset, then normalizes them into a shared holdings frame
...
- Shared helper functions are used across ETF endpoints so holdings, price-series, and top-holdings calculations stay consistent.
